...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.downsample, residual.shape, out.shape)
...
def squeezenet(pretrained=True, input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    return SqueezeNet(pretrained)


def densenet(pretrained=True,  input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    return DenseNet(pretrained=pretrained)


def resnet18(pretrained=True, input_channel=3):
    model = ResNet(BasicBlock, [2, 2, 2, 2], input_channel=input_channel)
    # print(model)
    # exit(0)
    if pretrained:
        if input_channel > 3:
            ...
            # import pdb; pdb.set_trace()
            model.conv1.weight.data[:,:3,...] = state_dict['conv1.weight'].data
...
def resnet34(pretrained=True, input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    model = ResNet(BasicBlock, [3, 4, 6, 3], input_channel=input_channel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(pretrained=True, input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    model = ResNet(Bottleneck, [3, 4, 6, 3], input_channel=input_channel)
    if pretrained:
...
def resnet101(pretrained=True, input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    model = ResNet(Bottleneck, [3, 4, 23, 3], input_channel=input_channel)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152(pretrained=True, input_channel=3):
    if input_channel != 3:
        raise NotImplementedError
    model = ResNet(Bottleneck, [3, 8, 36, 3], input_channel=input_channel)
    if pretrained:
