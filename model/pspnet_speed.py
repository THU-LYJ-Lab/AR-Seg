...
class PSPNetWithFuse(nn.Module):
    ...
    def forward(self, x, mode='normal', ref_p=None):
        ...
        if mode=='normal':
            ...
            return out, time_list, p
        
        elif mode=='mergespeed':
            # import pdb; pdb.set_trace()
            time_list = []
            N,C,H,W = x.shape
        ...
        elif mode=='fusionspeed':
            # import pdb; pdb.set_trace()
            time_list = []
            N,C,H,W = x.shape

            start = time.time()
            p = self.fuse_attention(ref_p, x)
            time_list.append(time.time()-start)
...
if __name__ == "__main__":
    ...
    # model = PSPNet(sizes=(1, 2, 3, 6), n_classes=12, psp_size=512, deep_features_size=256, backend='resnet18').cuda().eval()
    model = PSPNetWithFuse(sizes=(1, 2, 3, 6), n_classes=12, psp_size=512, deep_features_size=256, backend='resnet18').cuda().eval()
    size = [360, 480]
    input = torch.randn(1, 3, size[0], size[1]).cuda()
    input2 = torch.randn(1, 64, 720, 960).cuda()
    input3 = torch.randn(1, 64, 360, 480).cuda()

    time_list = []
    for i in range(100):
        start = time.time()
        # import pdb; pdb.set_trace()
        output = model(input,mode='mergespeed',ref_p=input2)
        # start = time.time()
        # output = model(output[1],mode='fusionspeed',ref_p=input2)
        # output = model(input,mode='speed')
        # time_list.append(output[-1])
        time_list.append(time.time()-start)
    # print(time_list)
    print(size, np.array(time_list).mean(axis=0).sum())
